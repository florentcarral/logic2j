...
/**
 * Prolog's most classic way of providing {@link Clause}s to the
 * {@link GoalSolver} inference engine: all clauses are parsed and normalized
 * from one or several theories' textual content managed by this class. TODO
 * Does the name "Manager" make sense here?
 */
public class DefaultTheoryManager implements TheoryManager {
    ...
    public TheoryContent load(PLibrary theLibrary) {
        ...
        if (contentUrl != null) {
            ...
            if (text instanceof InputStream) {
                // FIXME: there will be encoding issues when using InputStream
                // instead of Reader
                Reader reader = new InputStreamReader((InputStream) text);
                return load(reader);
            }
            throw new InvalidTermException("Could not load library from classloadable resource " + name
                    + ": could not getContent()");
        }
    ...
    /**
     * @param theContent
     *            to set - will replace any previously defined content.
     */
    @Override
    ...
    // TODO => Check if the parameter theGoalBindings may be used because up to
    // now it is declared in that method only because it has to implement it
    // with those parameters (cf ClauseProvider).
    /**
     * @param theGoal
     * @return All {@link Clause}s from the {@link TheoryContent} that may match
     *         theGoal.
     * @param theGoalBindings
     *            : is not used in that method, so you can give null as a
     *            parameter.
     */
    @Override
